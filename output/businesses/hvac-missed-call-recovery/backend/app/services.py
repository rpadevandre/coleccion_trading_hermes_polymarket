"""Business services for HVAC Missed-Call Recovery."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['CallIntakeService', 'UrgencyClassifier', 'RevenueEstimator', 'DispatchRouter', 'AuditLogService']
EXPECTED_SIGNALS = ['call_time', 'service_type', 'after_hours', 'zip_code', 'customer_message']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="HVAC Missed-Call Recovery assessment for missed calls, after-hours requests, and unprioritized emergency leads.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_dispatch_recovery_queue",
            )
        )
    return items
