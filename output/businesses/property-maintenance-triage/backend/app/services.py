"""Business services for Property Maintenance Triage."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['RequestIntakeService', 'IssueClassifier', 'VendorRoutingEngine', 'TenantSummaryWriter', 'SLAWatcher']
EXPECTED_SIGNALS = ['unit_id', 'issue_category', 'photos_available', 'after_hours', 'tenant_message']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="Property Maintenance Triage assessment for vague tenant repair requests, urgency confusion, and vendor routing overhead.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_maintenance_triage_queue",
            )
        )
    return items
