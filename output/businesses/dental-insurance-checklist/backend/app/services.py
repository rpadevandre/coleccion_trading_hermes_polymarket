"""Business services for Dental Insurance Checklist."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['VerificationIntakeService', 'ChecklistBuilder', 'PolicyLanguageSummarizer', 'ComplianceGuard', 'VerificationAuditService']
EXPECTED_SIGNALS = ['payer', 'procedure_type', 'appointment_date', 'missing_fields', 'verification_notes']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="Dental Insurance Checklist assessment for insurance verification mistakes that delay care, claims, and revenue collection.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_verification_review_queue",
            )
        )
    return items
