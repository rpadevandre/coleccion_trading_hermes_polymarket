"""Business services for B2B Podcast Repurposing System."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['TranscriptIngestionService', 'InsightExtractor', 'ChannelDraftGenerator', 'VoiceConsistencyScorer', 'PublishingQueueService']
EXPECTED_SIGNALS = ['episode_title', 'transcript_summary', 'target_channel', 'buyer_persona', 'campaign_goal']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="B2B Podcast Repurposing System assessment for valuable podcast/call transcripts that do not become reusable sales/content assets.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_content_asset_queue",
            )
        )
    return items
