"""Business services for Med Spa Lead Recovery."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['LeadIntakeService', 'TreatmentIntentClassifier', 'FollowupDraftService', 'ComplianceLanguageGuard', 'ConsultRevenueTracker']
EXPECTED_SIGNALS = ['lead_source', 'treatment_interest', 'message', 'last_contacted_at', 'booking_status']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="Med Spa Lead Recovery assessment for consult requests across calls, forms, DMs, and SMS that never become bookings.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_consult_recovery_queue",
            )
        )
    return items
