"""Business services for Chiropractic Reactivation Engine."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['PatientListImportService', 'ReactivationSegmenter', 'MessageDraftService', 'OptOutSuppressionService', 'RecoveredAppointmentTracker']
EXPECTED_SIGNALS = ['last_visit_date', 'patient_status', 'preferred_channel', 'care_plan_stage', 'opt_out_status']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="Chiropractic Reactivation Engine assessment for inactive patients who could return but are not followed up systematically.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_reactivation_review_queue",
            )
        )
    return items
