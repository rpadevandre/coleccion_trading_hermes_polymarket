"""Business services for Law Firm Intake Triage."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['LegalInquiryIntakeService', 'PracticeAreaClassifier', 'ConflictChecklistService', 'AttorneySummaryWriter', 'IntakeAuditService']
EXPECTED_SIGNALS = ['practice_area', 'deadline', 'jurisdiction', 'opposing_party', 'inquiry_summary']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="Law Firm Intake Triage assessment for poor-fit inquiries and urgent potential cases mixed together in intake channels.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_attorney_review_queue",
            )
        )
    return items
