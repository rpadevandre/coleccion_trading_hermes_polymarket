# Law Firm Intake Triage — FastAPI Backend

    This is the first executable backend scaffold for **Law Firm Intake Triage**.
    ...
    ```bash
    cd output/businesses/law-firm-intake-triage/backend
    python -m venv .venv
    . .venv/bin/activate
    ...
    ```text
    database: law_firm_intake_triage
    intakes: intake_case_intakes
    assessments: intake_case_assessments
    audit_events: audit_events
    ```

    ## Business modules represented

    - `LegalInquiryIntakeService`
- `PracticeAreaClassifier`
- `ConflictChecklistService`
- `AttorneySummaryWriter`
- `IntakeAuditService`
