"""Business services for MSP Security Reporting."""
from app.repository import repository
from app.schemas import AssessmentRequest, AssessmentResult, IntakeCreate, IntakeRecord, Priority, QueueItem

AI_MODULES = ['EvidenceImportService', 'SignalNormalizer', 'RiskNarrativeWriter', 'ClientReportGenerator', 'ApprovalAuditService']
EXPECTED_SIGNALS = ['client_name', 'finding_type', 'severity', 'evidence_summary', 'recommended_action']
...
def assess(payload: AssessmentRequest) -> AssessmentResult:
    ...
    result = AssessmentResult(
        ...
        score=score,
        priority=priority,
        summary="MSP Security Reporting assessment for technical security evidence that is hard to convert into client-friendly reporting.",
    ...
    for intake in repository.list_intakes():
        items.append(
            QueueItem(
                ...
                next_action="run_assessment_then_route_to_reporting_review_queue",
            )
        )
    return items
